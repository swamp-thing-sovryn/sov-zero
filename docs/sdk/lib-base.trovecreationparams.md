...
```typescript
export declare type TroveCreationParams<T = unknown> = _CollateralDeposit<T> & _NoCollateralWithdrawal & _ZSUSDBorrowing<T> & _NoZSUSDRepayment;
```
...
<tr> <td> borrowZSUSD </td> <td> T </td> <td> The amount of ZSUSD that's borrowed. </td> </tr>
