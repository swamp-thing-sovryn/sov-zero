...
Defining both `depositCollateral` and `withdrawCollateral`<!-- -->, or both `borrowZSUSD` and `repayZSUSD` at the same time is disallowed, and will result in a type-checking error.
...
<tr> <td> borrowZSUSD? </td> <td> T </td> <td> <i>(Optional)</i> The amount of ZSUSD that's borrowed. </td> </tr>

<tr> <td> repayZSUSD? </td> <td> T </td> <td> <i>(Optional)</i> The amount of ZSUSD that's repaid. </td> </tr>
