 ...
 * --- IMPLEMENTATION ---
 *
 * We use a highly scalable method of tracking deposits and SOV gains that has O(1) complexity.
 *
 * When a liquidation occurs, rather than updating each depositor's deposit and SOV gain, we simply update two state variables:
 * a product P, and a sum S.
 *
 * A mathematical manipulation allows us to factor out the initial deposit, and accurately track all depositors' compounded deposits
 * and accumulated SOV gains over time, as liquidations occur, using just these two variables P and S. When depositors join the
 * Stability Pool, they get a snapshot of the latest P and S: P_t and S_t, respectively.
 *
 * The formula for a depositor's accumulated SOV gain is derived here:
 * https://github.com/liquity/dev/blob/main/packages/contracts/mathProofs/Scalable%20Compounding%20Stability%20Pool%20Deposits.pdf
 *
 * For a given deposit d_t, the ratio P/P_t tells us the factor by which a deposit has decreased since it joined the Stability Pool,
 * and the term d_t * (S - S_t)/P_t gives us the deposit's total accumulated SOV gain.
 *
 * Each liquidation updates the product P and sum S. After a series of liquidations, a compounded deposit and corresponding SOV gain
 * can be calculated using the initial deposit, the depositor’s snapshots of P and S, and the latest values of P and S.
 *
 * Any time a depositor updates their deposit (withdrawal, top-up) their accumulated SOV gain is paid out, their new deposit is recorded
 ...
 *
 * This allows us to calculate a deposit's accumulated SOV gain, during the epoch in which the deposit was non-zero and earned SOV.
 *
 * We calculate the depositor's accumulated SOV gain for the scale at which they made the deposit, using the SOV gain formula:
 * e_1 = d_t * (S - S_t) / P_t
 *
 ...
 *         i            i+1
 *
 * The sum of (e_1 + e_2) captures the depositor's total accumulated SOV gain, handling the case where their
...
contract StabilityPool is LiquityBase, StabilityPoolStorage, CheckContract, IStabilityPool {
    ...
    * - Updates snapshots for deposit and tagged front end stake */
    function withdrawSOVGainToTrove(address _upperHint, address _lowerHint) external override {
        uint initialDeposit = deposits[msg.sender].initialValue;
        _requireUserHasDeposit(initialDeposit);
    ...
    */
    ...
    function _updateRewardSumAndProduct(uint _SOVGainPerUnitStaked, uint _ZUSDLossPerUnitStaked) internal {
        ...
        * The SOV gain for any given depositor from a liquidation depends on the value of their deposit
        * (and the value of totalDeposits) prior to the Stability being depleted by the debt in the liquidation.
        *
        * Since S corresponds to SOV gain, and P to deposit loss, we update S first.
        */
        uint marginalSOVGain = _SOVGainPerUnitStaked.mul(currentP);
        uint newS = currentS.add(marginalSOVGain);
    ...
    /** Calculates the SOV gain earned by the deposit since its last snapshots were taken.
    * Given by the formula:  E = d0 * (S - S(0))/P(0)
    * where S(0) and P(0) are the depositor's snapshots of the sum S and product P, respectively.
    * d0 is the last recorded deposit value.
    */
    function getDepositorSOVGain(address _depositor) public view override returns (uint) {
        ...
        uint SOVGain = _getSOVGainFromSnapshots(initialDeposit, snapshots);
        return SOVGain;
    }

    function _getSOVGainFromSnapshots(uint initialDeposit, Snapshots memory snapshots) internal view returns (uint) {
        /*
        * Grab the sum 'S' from the epoch at which the stake was made. The SOV gain may span up to one scale change.
    ...
    */
    ...
    function _sendSOVGainToDepositor(uint _amount) internal {
        if (_amount == 0) {return;}
        uint newSOVBalance = sovToken.balanceOf(address(this)).sub(_amount);
        emit StabilityPoolSOVBalanceUpdated(newSOVBalance);
        emit SOVSent(msg.sender, _amount);
    ...
    function _requireUserHasTrove(address _depositor) internal view {
        require(troveManager.getTroveStatus(_depositor) == 1, "StabilityPool: caller must have an active trove to withdraw SOVGain to");
    }

    function _requireUserHasSOVGain(address _depositor) internal view {
        uint SOVGain = getDepositorSOVGain(_depositor);
        require(SOVGain > 0, "StabilityPool: caller must have non-zero SOV Gain");
    }
