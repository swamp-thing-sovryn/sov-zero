...
contract TroveManagerBase is LiquityBase, TroveManagerStorage {
    ...
    function _getPendingSOVReward(address _borrower) public view returns (uint256) {
        uint256 snapshotSOV = rewardSnapshots[_borrower].SOV;
        uint256 rewardPerUnitStaked = L_SOV.sub(snapshotSOV);
        ...
        uint256 pendingSOVReward = stake.mul(rewardPerUnitStaked).div(DECIMAL_PRECISION);
    ...
    function _hasPendingRewards(address _borrower) public view returns (bool) {
        ...
        return (rewardSnapshots[_borrower].SOV < L_SOV);
    }
    ...
    function _calcRedemptionFee(uint256 _redemptionRate, uint256 _SOVDrawn)
        internal
        pure
        returns (uint256)
    {
        uint256 redemptionFee = _redemptionRate.mul(_SOVDrawn).div(DECIMAL_PRECISION);
        require(redemptionFee < _SOVDrawn, "TroveManager: Fee would eat up all returned collateral");
    ...
    function _getRedemptionFee(uint256 _SOVDrawn) internal view returns (uint256) {
        return _calcRedemptionFee(_getRedemptionRate(), _SOVDrawn);
    }
