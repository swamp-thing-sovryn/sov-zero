...
contract TroveManagerRedeemOps is TroveManagerBase {
    ...
    function redeemCollateral(
        ...
        emit Redemption(_ZUSDamount, totals.totalZUSDToRedeem, totals.totalSOVDrawn, totals.SOVFee);

        // Burn the total ZUSD that is cancelled with debt, and send the redeemed SOV to msg.sender
        contractsCache.zusdToken.burn(msg.sender, totals.totalZUSDToRedeem);
        // Update Active Pool ZUSD, and send SOV to account
    ...
    function _redeemCollateralFromTrove(
        ...
        singleRedemption.SOVLot = singleRedemption.ZUSDLot.mul(DECIMAL_PRECISION).div(_price);

        // Decrease the debt and collateral of the current Trove according to the ZUSD lot and corresponding SOV to send
    ...
    function _updateBaseRateFromRedemption(
        ...
        /* Convert the drawn SOV back to ZUSD at face value rate (1 ZUSD:1 USD), in order to get
         * the fraction of total supply that was redeemed at face value. */
        uint256 redeemedZUSDFraction = _SOVDrawn.mul(_price).div(_totalZUSDSupply);
    ...
    /**
      ...
      The redeemer swaps (debt - liquidation reserve) ZUSD for (debt - liquidation reserve) worth of SOV, so the ZUSD liquidation reserve left corresponds to the remaining debt.
