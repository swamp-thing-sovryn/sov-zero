...
ZSUSD_GAS_COMPENSATION = 200.0
MIN_NET_DEBT = 1800.0
MAX_FEE = Wei(1e18)

"""# Ether price (exogenous)

Ether is the collateral for ZSUSD. The ether price $P_t^e$ follows 
...
> (collateral in ether, debt in ZSUSD, collateral ratio target, rational inattention, collateral ratio)
...
$P_{t}^{l}$ is the price of ZSUSD.

$N_t^o$ is rounded to an integer.

---

The amount of ZSUSD tokens generated by a new trove is
...
- $D_{t}^{s}$ is the total amount of ZSUSD deposited in the Stability Pool (see below)
...
def pending_liquidations(contracts, price_ether_current):
    ...
    stability_pool_balance = contracts.stabilityPool.getTotalZSUSDDeposits()
    trove = last_trove
...
def liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_ZSUSD, price_ZERO_current, data, index):
    if len(active_accounts) == 0:
        return [0, 0]

    stability_pool_previous = contracts.stabilityPool.getTotalZSUSDDeposits() / 1e18
    ...
    while pending_liquidations(contracts, price_ether_current):
        try:
            ...
            for i in range(NUM_LIQUIDATIONS):
                ...
                ICR = contracts.troveManager.getCurrentICR(trove, Wei(price_ether_current * 1e18))
                print(f"ICR: {ICR}")
    stability_pool_current = contracts.stabilityPool.getTotalZSUSDDeposits() / 1e18
    ...
    liquidation_gain = ether_liquidated * price_ether_current - debt_liquidated * price_ZSUSD
    ...
    return_stability = calculate_stability_return(contracts, price_ZSUSD, data, index)

    return [ether_liquidated, return_stability]

def calculate_stability_return(contracts, price_ZSUSD, data, index):
    stability_pool_previous = contracts.stabilityPool.getTotalZSUSDDeposits() / 1e18
    if index == 0:
        ...
        return_stability = (year/index) * \
            (sum(data['liquidation_gain'][0:index]) +
             ...
             ) / (price_ZSUSD * stability_pool_previous)
    else:
...
def close_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_ZSUSD, index):
    if len(active_accounts) == 0:
        return [0]
    ...
    if index <= 240:
        ...
        number_closetroves = max(0, n_steady * (1+shock_closetroves)) + beta*(1-price_ZSUSD)*n_troves
    ...
    for i in range(0, len(drops)):
        ...
        coll = amounts['coll']
        debt = amounts['debt']
        pending = get_zsusd_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt)
        if pending == 0:
...
def transfer_from_to(contracts, from_account, to_account, amount):
    balance = contracts.zsusdToken.balanceOf(from_account)
    transfer_amount = min(balance, amount)
    if transfer_amount == 0:
        return amount
    if from_account == to_account:
        return amount
    contracts.zsusdToken.transfer(to_account, transfer_amount, { 'from': from_account })
    pending = amount - transfer_amount

    return pending

def get_zsusd_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt):
    zsusdBalance = contracts.zsusdToken.balanceOf(account)
    if debt > zsusdBalance:
        pending = debt - zsusdBalance
        ...
        # try with active accounts, which are more likely to hold ZSUSD
        ...
        if pending > 0:
            print(f"\n ***Error: not enough ZSUSD to repay! {debt / 1e18} ZSUSD for {account}")
...
def adjust_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, index):
    ...
    for i, working_trove in enumerate(active_accounts):
        ...
        if p >= ratio:
            ...
            if check < -1:
                ...
                pending = get_zsusd_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, repay_amount)
                if pending == 0:
                    contracts.borrowerOperations.repayZSUSD(repay_amount, hints[0], hints[1], { 'from': account })
                ...
                if isNewTCRAboveCCR(contracts, 0, False, withdraw_amount_wei, True, floatToWei(price_ether_current)):
                    contracts.borrowerOperations.withdrawZSUSD(MAX_FEE, withdraw_amount_wei, hints[0], hints[1], { 'from': account })
                    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
                    issuance_ZSUSD_adjust = issuance_ZSUSD_adjust + rate_issuance * withdraw_amount
    ...
    return [coll_added_float, issuance_ZSUSD_adjust]
...
def open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention, price_ether_current):
    ...
    debtChange = floatToWei(supply_trove) + ZSUSD_GAS_COMPENSATION
    zsusd = get_zsusd_amount_from_net_debt(contracts, floatToWei(supply_trove))
    if isNewTCRAboveCCR(contracts, coll, True, debtChange, True, floatToWei(price_ether_current)):
        contracts.borrowerOperations.openTrove(MAX_FEE, zsusd, hints[0], hints[1],
...
def open_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_ZSUSD, index):
    ...
    elif price_ZSUSD <= 1 + rate_issuance:
        number_opentroves = max(0, n_steady * (1+shock_opentroves))
    else:
        number_opentroves = max(0, n_steady * (1+shock_opentroves)) + \
                        alpha * (price_ZSUSD - rate_issuance - 1) * n_troves
    ...
    return [coll_added, issuance_ZSUSD_open]
...
def stability_update(accounts, contracts, active_accounts, return_stability, index):
    supply = contracts.zsusdToken.totalSupply() / 1e18
    stability_pool_previous = contracts.stabilityPool.getTotalZSUSDDeposits() / 1e18
    ...
    if stability_pool > stability_pool_previous:
        ...
        while remaining > 0 and i < len(active_accounts):
          ...
          balance = contracts.zsusdToken.balanceOf(account) / 1e18
        ...
        current_deposit = contracts.stabilityPool.getCompoundedZSUSDDeposit(accounts[0])
...
---
With the supply and demand of ZSUSD tokens defined above, the price of ZSUSD at the current period is given by the following equilibrium condition:
> $$S_t = D_t^s + D_t^l = D_t^s + D_{t-1}^l (1+\zeta_t^l)(1+\sigma_t^l) (\frac{P_t^l}{P_{t-1}^l})^\delta$$

where $S$ is the total supply of ZSUSD.

Solving this equation gives that:
> $$P_t^l = P_{t-1}^l (\frac{S_t-D_t^s}{D_{t-1}^l(1+\zeta_t^l)(1+\sigma_t^l)})^{1/\delta}$$
"""

def calculate_price(price_ZSUSD, liquidity_pool, liquidity_pool_next):
  # liquidity_pool = supply - stability_pool
  # liquidity_pool_next = liquidity_pool_previous * drift_liquidity * (1+shock_liquidity)
  price_ZSUSD_current= price_ZSUSD * (liquidity_pool / liquidity_pool_next) ** (1/delta)

  return price_ZSUSD_current

""" **Stabilizers**

There are two stabilizers to attenuate ZSUSD price deviation from its target range.
...
The amount of $Q_t^d(c)$ is expected to bring the ZSUSD price back to $1.1+f_t^i$. This means that
...
If $P_t^l < 1-f_t^r$, a fraction $\chi_t$ of ZSUSD in the liquidity pool is used for redemption
> $$D_t^r = \chi_t D_t^l,$$
...
Note that unlike stabilizer 1, stabilizer 2 has impact of ZSUSD price in
...
def redeem_trove(accounts, contracts, i, price_ether_current):
    zsusd_balance = contracts.zsusdToken.balanceOf(accounts[i])
    [firstRedemptionHint, partialRedemptionHintNICR, truncatedZSUSDamount] = contracts.hintHelpers.getRedemptionHints(zsusd_balance, price_ether_current, 70)
    if truncatedZSUSDamount == Wei(0):
        return None
    ...
    try:
        ...
        print(f"ZSUSD Token:    {contracts.zsusdToken.address}")
        print(f"i: {i}")
        print(f"account: {accounts[i]}")
        print(f"ZSUSD bal: {zsusd_balance / 1e18}")
        print(f"truncated: {truncatedZSUSDamount / 1e18}")
        ...
        print(f"amount: {truncatedZSUSDamount}")
...
def price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_ZSUSD, index):

    stability_pool = contracts.stabilityPool.getTotalZSUSDDeposits() / 1e18
    redemption_pool = 0
    redemption_fee = 0
    ...
    #Calculating Price
    price_ZSUSD_current = calculate_price(price_ZSUSD, liquidity_pool, liquidity_pool_next)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    ...
    if price_ZSUSD_current > 1.1 + rate_issuance:
        supply_wanted = stability_pool + \
                        liquidity_pool_next * \
                        ((1.1+rate_issuance) / price_ZSUSD)**delta
    ...
    if price_ZSUSD_current < 1 - rate_redemption:
        ...
        supply_target = stability_pool + \
                        liquidity_pool_next * \
                        ((1-rate_redemption) / price_ZSUSD)**delta
    ...
    return [price_ZSUSD_current, redemption_pool, redemption_fee, issuance_ZSUSD_stabilizer]
