...
contract('TroveManager - in Recovery Mode', async accounts => {
  ...
  it("checkRecoveryMode(): Returns true if TCR falls below CCR", async () => {
    ...
    // price drops to 1SOV:150ZSUSD, reducing TCR below 150%.  setPrice() calls checkTCRAndSetRecoveryMode() internally.
  ...
  it("checkRecoveryMode(): Returns true if TCR stays less than CCR", async () => {
    ...
    // price drops to 1SOV:150ZSUSD, reducing TCR below 150%
    await priceFeed.setPrice('150000000000000000000')

    const recoveryMode_Before = await th.checkRecoveryMode(contracts);
    assert.isTrue(recoveryMode_Before)

    await sovToken.approve(borrowerOperations.address, '1', { from: alice })
  ...
  it("checkRecoveryMode(): returns false if TCR rises above CCR", async () => {
    ...
    await sovToken.approve(borrowerOperations.address, A_coll, { from: alice })
  ...
  it("liquidate(), with ICR < 100%: removes stake and updates totalStakes", async () => {
    ...
    //  Alice and Bob withdraw such that the TCR is ~150%
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraZSUSDAmount: dec(2000, 18), extraParams: { from: bob } })
    ...
    // --- TEST ---
    // price drops to 1SOV:100ZSUSD, reducing TCR below 150%
  ...
  it("liquidate(), with ICR < 100%: updates system snapshots correctly", async () => {
    ...
    // --- TEST ---
    // price drops to 1SOV:100ZSUSD, reducing TCR below 150%, and all Troves below 100% ICR
    await priceFeed.setPrice('100000000000000000000')
  ...
  it("liquidate(), with ICR < 100%: closes the Trove and removes it from the Trove array", async () => {
    ...
    //  Alice and Bob withdraw such that the TCR is ~150%
    await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(150, 16)), extraZSUSDAmount: dec(2000, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with ICR < 100%: only redistributes to active Troves - no offset to Stability Pool", async () => {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraZSUSDAmount: spDeposit, extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: dennis } })
  ...
  it("liquidate(), with 100 < ICR < 110%: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    //  Bob withdraws up to 2000 ZSUSD of debt, bringing his ICR to 210%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(210, 16)), extraZSUSDAmount: dec(2000, 18), extraParams: { from: bob } })
    ...
    // --- TEST ---
    // price drops to 1SOV:100ZSUSD, reducing TCR to 117%
    await priceFeed.setPrice('100000000000000000000')
  ...
  it("liquidate(), with 100% < ICR < 110%: updates system snapshots correctly", async () => {
    ...
    //  Bob withdraws up to 20000 ZSUSD of debt, bringing his ICR to 210%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(210, 16)), extraZSUSDAmount: dec(20000, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: dennis } })
  ...
  it("liquidate(), with 100% < ICR < 110%: offsets as much debt as possible with the Stability Pool, then redistributes the remainder coll and debt", async () => {
    ...
    // Alice deposits 390ZSUSD to the Stability Pool
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: alice })
    ...
    // check pool ZSUSD before liquidation
    const stabilityPoolZSUSD_Before = (await stabilityPool.getTotalZSUSDDeposits()).toString()
    assert.equal(stabilityPoolZSUSD_Before, '390000000000000000000')
    ...
    /* Now, liquidate Bob. Liquidated coll is 21 ether, and liquidated debt is 2000 ZSUSD.
    
    With 390 ZSUSD in the StabilityPool, 390 ZSUSD should be offset with the pool, leaving 0 in the pool.
  
    Stability Pool rewards for alice should be:
    ZSUSDLoss: 390ZSUSD
    SOVGain: (390 / 2000) * 21*0.995 = 4.074525 ether

    After offsetting 390 ZSUSD and 4.074525 ether, the remainders - 1610 ZSUSD and 16.820475 ether - should be redistributed to all active Troves.
   */
    // Liquidate Bob
    await troveManager.liquidate(bob, { from: owner })

    const aliceDeposit = await stabilityPool.getCompoundedZSUSDDeposit(alice)
    const aliceSOVGain = await stabilityPool.getDepositorSOVGain(alice)
    const aliceExpectedSOVGain = spDeposit.mul(th.applyLiquidationFee(B_coll)).div(B_totalDebt)

    assert.equal(aliceDeposit.toString(), 0)
    assert.equal(aliceSOVGain.toString(), aliceExpectedSOVGain)

    /* Now, check redistribution to active Troves. Remainders of 1610 ZSUSD and 16.82 ether are distributed.
    ...
    L_ZSUSDDebt = 1610 / 6 = 268.333 ZSUSD
    L_SOV = 16.820475 /6 =  2.8034125 ether
    */
    const L_ZSUSDDebt = (await troveManager.L_ZSUSDDebt()).toString()
    const L_SOV = (await troveManager.L_SOV()).toString()

    assert.isAtMost(th.getDifference(L_ZSUSDDebt, B_totalDebt.sub(spDeposit).mul(mv._1e18BN).div(A_coll.add(D_coll))), 100)
    assert.isAtMost(th.getDifference(L_SOV, th.applyLiquidationFee(B_coll.sub(B_coll.mul(spDeposit).div(B_totalDebt)).mul(mv._1e18BN).div(A_coll.add(D_coll)))), 100)
  })
  ...
  it("liquidate(), with ICR > 110%, trove has lowest ICR, and StabilityPool is empty: does nothing", async () => {
    ...
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraZSUSDAmount: dec(2000, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZSUSDAmount: dec(2000, 18), extraParams: { from: dennis } })
    ...
    assert.equal(L_ZSUSDDebt, '0')
    assert.equal(L_SOV, '0')
  ...
  // --- liquidate(), applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool ZSUSD is GREATER THAN liquidated debt ---

  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ZSUSD > debt to liquidate: offsets the trove entirely with the pool", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZSUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZSUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraZSUSDAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZSUSDAmount: B_totalDebt, extraParams: { from: alice } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZSUSDAmount: dec(2000, 18), extraParams: { from: dennis } })

    // Alice deposits ZSUSD in the Stability Pool
    ...
    /* Check accrued Stability Pool rewards after. Total Pool deposits was 1490 ZSUSD, Alice sole depositor.
    As liquidated debt (250 ZSUSD) was completely offset

    Alice's expected compounded deposit: (1490 - 250) = 1240ZSUSD
    Alice's expected SOV gain:  Bob's liquidated capped coll (minus gas comp), 2.75*0.995 ether
  
    */
    const aliceExpectedDeposit = await stabilityPool.getCompoundedZSUSDDeposit(alice)
    const aliceExpectedSOVGain = await stabilityPool.getDepositorSOVGain(alice)

    assert.isAtMost(th.getDifference(aliceExpectedDeposit.toString(), spDeposit.sub(B_totalDebt)), 2000)
    assert.isAtMost(th.getDifference(aliceExpectedSOVGain, th.applyLiquidationFee(B_totalDebt.mul(th.toBN(dec(11, 17))).div(price))), 3000)
    ...
    const bob_balanceBefore = await sovToken.balanceOf(bob)
    await borrowerOperations.claimCollateral({ from: bob, gasPrice: 0 })
    const bob_balanceAfter = await sovToken.balanceOf(bob)
    th.assertIsApproximatelyEqual(bob_balanceAfter, bob_balanceBefore.add(th.toBN(bob_remainingCollateral)))
  })

  it("liquidate(), with ICR% = 110 < TCR, and StabilityPool ZSUSD > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZSUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZSUSD of debt, resulting in ICR of 220%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(220, 16)), extraZSUSDAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZSUSDAmount: B_totalDebt, extraParams: { from: alice } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZSUSDAmount: dec(2000, 18), extraParams: { from: dennis } })
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool ZSUSD > debt to liquidate: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZSUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZSUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraZSUSDAmount: dec(250, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool ZSUSD > debt to liquidate: updates system snapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZSUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZSUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraZSUSDAmount: dec(250, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ZSUSD > debt to liquidate: closes the Trove", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZSUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZSUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraZSUSDAmount: dec(250, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ZSUSD > debt to liquidate: can liquidate troves out of order", async () => {
    // taking out 1000 ZSUSD, CR of 200%
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraZSUSDAmount: totalLiquidatedDebt, extraParams: { from: whale } })
    ...
    // can claim collateral
    const dennis_balanceBefore = await sovToken.balanceOf(dennis)
    await borrowerOperations.claimCollateral({ from: dennis, gasPrice: 0 })
    const dennis_balanceAfter = await sovToken.balanceOf(dennis)
    ...
    const carol_balanceBefore = await sovToken.balanceOf(carol)
    await borrowerOperations.claimCollateral({ from: carol, gasPrice: 0 })
    const carol_balanceAfter = await sovToken.balanceOf(carol)
  ...
  ZSUSD is LESS THAN the liquidated debt: a non fullfilled liquidation --- */

  it("liquidate(), with ICR > 110%, and StabilityPool ZSUSD < liquidated debt: Trove remains active", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZSUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZSUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZSUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(240, 16)), extraZSUSDAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZSUSDAmount: dec(2000, 18), extraParams: { from: dennis } })

    // Alice deposits 100 ZSUSD in the Stability Pool
    // Note:  originally the test was assigning 1490 ZSUSD to the pool but later it states the SP only has 100 ZSUSD
    //        and actually tests what's expected, that the SP doesn't have enough ZSUSD to pay for Bob debt so does nothing 
    ...
    /* Since the pool only contains 100 ZSUSD, and Bob's pre-liquidation debt was 250 ZSUSD,
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ZSUSD < liquidated debt: Trove remains in TroveOwners array", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZSUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZSUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZSUSDAmount: dec(1500, 18), extraParams: { from: alice } })
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ZSUSD < liquidated debt: nothing happens", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZSUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZSUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZSUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    /*  Since Bob's debt (250 ZSUSD) is larger than all ZSUSD in the Stability Pool, Liquidation won’t happen
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ZSUSD < liquidated debt: updates system shapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZSUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZSUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZSUSDAmount: dec(1500, 18), extraParams: { from: alice } })
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ZSUSD < liquidated debt: causes correct Pool offset and SOV gain, and doesn't redistribute to active troves", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZSUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZSUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZSUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    assert.equal(aliceExpectedSOVGain.toString(), '0')
    ...
    const L_ZSUSDDebt_After = (await troveManager.L_ZSUSDDebt()).toString()
    const L_SOV_After = (await troveManager.L_SOV()).toString()

    assert.equal(L_ZSUSDDebt_After, '0')
    assert.equal(L_SOV_After, '0')
  })

  it("liquidate(), with ICR > 110%, and StabilityPool ZSUSD < liquidated debt: ICR of non liquidated trove does not change", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZSUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZSUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    // Carol withdraws up to debt of 240 ZSUSD, -> ICR of 250%.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZSUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(240, 16)), extraZSUSDAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZSUSDAmount: dec(2000, 18), extraParams: { from: dennis } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(250, 16)), extraZSUSDAmount: dec(240, 18), extraParams: { from: carol } })
    ...
    //Check SP ZSUSD has been completely emptied
    assert.equal((await stabilityPool.getTotalZSUSDDeposits()).toString(), dec(100, 18))
    ...
    await zsusdToken.transfer(bob, dec(100, 18), { from: alice })
    ...
    // Alice provides another 50 ZSUSD to pool
    ...
    //Check SP ZSUSD has been completely emptied
    assert.equal((await stabilityPool.getTotalZSUSDDeposits()).toString(), dec(150, 18))
  ...
  it("liquidate() with ICR > 110%, and StabilityPool ZSUSD < liquidated debt: total liquidated coll and debt is correct", async () => {
    // Whale provides 50 ZSUSD to the SP
    await openTrove({ ICR: toBN(dec(300, 16)), extraZSUSDAmount: dec(50, 18), extraParams: { from: whale } })
  ...
  it("liquidate(): Doesn't liquidate undercollateralized trove if it is the only trove in the system", async () => {
    // Alice creates a single trove with 0.62 SOV and a debt of 62 ZSUSD, and provides 10 ZSUSD to SP
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    /*  Since there is 0 ZSUSD in the stability Pool, A, with ICR >110%, should stay active.
  ...
  it("liquidate(): does not affect the SP deposit or SOV gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await openTrove({ ICR: toBN(dec(220, 16)), extraZSUSDAmount: spDeposit, extraParams: { from: bob } })

    // Bob sends tokens to Dennis, who has no trove
    await zsusdToken.transfer(dennis, spDeposit, { from: bob })

    //Dennis provides 200 ZSUSD to SP
    ...
    assert.isAtMost(th.getDifference(dennis_SOVGain_Before, th.applyLiquidationFee(C_coll)), 1000)
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(220, 16)), extraZSUSDAmount: dec(1000, 18), extraParams: { from: whale } })

    const { zsusdAmount: A_zsusdAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraZSUSDAmount: dec(300, 18), extraParams: { from: alice } })
    const { zsusdAmount: B_zsusdAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraZSUSDAmount: dec(200, 18), extraParams: { from: bob } })
    const { zsusdAmount: C_zsusdAmount } = await openTrove({ ICR: toBN(dec(206, 16)), extraZSUSDAmount: dec(100, 18), extraParams: { from: carol } })
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, re-open, be reedemed and claim again", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZSUSD of debt, resulting in ICRs of 266%.
    // Bob withdraws up to 480 ZSUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraZSUSDAmount: dec(480, 18), extraParams: { from: bob } })
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZSUSDAmount: B_totalDebt, extraParams: { from: alice } })
    ...
    // Bob re-opens the trove, price 200, total debt 80 ZSUSD, ICR = 120% (lowest one)
    // Dennis redeems 30, so Bob has a surplus of (200 * 0.48 - 30) / 200 = 0.33 SOV
    await priceFeed.setPrice('200000000000000000000')
    const { collateral: B_coll_2, netDebt: B_netDebt_2 } = await openTrove({ ICR: toBN(dec(150, 16)), extraZSUSDAmount: dec(480, 18), extraParams: { from: bob, value: bob_remainingCollateral } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZSUSDAmount: B_netDebt_2, extraParams: { from: dennis } })
    ...
    // can claim collateral
    const bob_balanceBefore_2 = await sovToken.balanceOf(bob)
    await borrowerOperations.claimCollateral({ from: bob, gasPrice: 0 })
    const bob_balanceAfter_2 = await sovToken.balanceOf(bob)
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, after another claim from a redemption", async () => {
    // --- SETUP ---
    // Bob withdraws up to 90 ZSUSD of debt, resulting in ICR of 222%
    const { collateral: B_coll, netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(222, 16)), extraZSUSDAmount: dec(90, 18), extraParams: { from: bob } })
    // Dennis withdraws to 150 ZSUSD of debt, resulting in ICRs of 266%.
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraZSUSDAmount: B_netDebt, extraParams: { from: dennis } })

    // --- TEST ---
    // skip bootstrapping phase
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_WEEK * 2, web3.currentProvider)

    // Dennis redeems 40, so Bob has a surplus of (200 * 1 - 40) / 200 = 0.8 SOV
    ...
    // Bob re-opens the trove, price 200, total debt 250 ZSUSD, ICR = 240% (lowest one)
    ...
    // Alice deposits ZSUSD in the Stability Pool
    await openTrove({ ICR: toBN(dec(266, 16)), extraZSUSDAmount: B_totalDebt_2, extraParams: { from: alice } })
  ...
  it("liquidateTroves(): With all ICRs > 110%, Liquidates Troves until system leaves recovery mode", async () => {
    ...
    // Everyone withdraws some ZSUSD from their Trove, resulting in different ICRs
    ...
    const { totalDebt: H_totalDebt } = await openTrove({ ICR: toBN(dec(222, 16)), extraZSUSDAmount: dec(5000, 16), extraParams: { from: harry } })
    const liquidationAmount = E_totalDebt.add(F_totalDebt).add(G_totalDebt).add(H_totalDebt)
    await openTrove({ ICR: toBN(dec(400, 16)), extraZSUSDAmount: liquidationAmount, extraParams: { from: alice } })

    // Alice deposits ZSUSD to Stability Pool
    await stabilityPool.provideToSP(liquidationAmount, ZERO_ADDRESS, { from: alice })

    // price drops
    // price drops to 1SOV:90ZSUSD, reducing TCR below 150%
  ...
  it("liquidateTroves(): Liquidates Troves until 1) system has left recovery mode AND 2) it reaches a Trove with ICR >= 110%", async () => {
    ...
    // price drops to 1SOV:85ZSUSD, reducing TCR below 150%
  ...
  it("liquidateTroves(): does nothing if n = 0", async () => {
    await openTrove({ ICR: toBN(dec(200, 16)), extraZSUSDAmount: dec(100, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraZSUSDAmount: dec(200, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraZSUSDAmount: dec(300, 18), extraParams: { from: carol } })
  ...
  it("liquidateTroves(): a liquidation sequence containing Pool offsets increases the TCR", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraZSUSDAmount: dec(500, 18), extraParams: { from: whale } })
    ...
    await openTrove({ ICR: toBN(dec(198, 16)), extraZSUSDAmount: dec(101, 18), extraParams: { from: defaulter_1 } })
    await openTrove({ ICR: toBN(dec(184, 16)), extraZSUSDAmount: dec(217, 18), extraParams: { from: defaulter_2 } })
    await openTrove({ ICR: toBN(dec(183, 16)), extraZSUSDAmount: dec(328, 18), extraParams: { from: defaulter_3 } })
    await openTrove({ ICR: toBN(dec(186, 16)), extraZSUSDAmount: dec(431, 18), extraParams: { from: defaulter_4 } })
    ...
    // Check Stability Pool has 500 ZSUSD
  ...
  it("liquidateTroves(): A liquidation sequence of pure redistributions decreases the TCR, due to gas compensation, but up to 0.5%", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(250, 16)), extraZSUSDAmount: dec(500, 18), extraParams: { from: whale } })
    ...
    const { collateral: d1_coll, totalDebt: d1_totalDebt } = await openTrove({ ICR: toBN(dec(198, 16)), extraZSUSDAmount: dec(101, 18), extraParams: { from: defaulter_1 } })
    const { collateral: d2_coll, totalDebt: d2_totalDebt } = await openTrove({ ICR: toBN(dec(184, 16)), extraZSUSDAmount: dec(217, 18), extraParams: { from: defaulter_2 } })
    const { collateral: d3_coll, totalDebt: d3_totalDebt } = await openTrove({ ICR: toBN(dec(183, 16)), extraZSUSDAmount: dec(328, 18), extraParams: { from: defaulter_3 } })
    const { collateral: d4_coll, totalDebt: d4_totalDebt } = await openTrove({ ICR: toBN(dec(166, 16)), extraZSUSDAmount: dec(431, 18), extraParams: { from: defaulter_4 } })
  ...
  it('liquidateTroves(): does nothing if all troves have ICR > 110% and Stability Pool is empty', async () => {
    ...
    // Confirm 0 ZSUSD in Stability Pool
    assert.equal((await stabilityPool.getTotalZSUSDDeposits()).toString(), '0')
  ...
  it('liquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves', async () => {
    ...
    // Whale adds ZSUSD to SP
    const spDeposit = F_totalDebt.add(G_totalDebt)
    await openTrove({ ICR: toBN(dec(285, 16)), extraZSUSDAmount: spDeposit, extraParams: { from: whale } })
    ...
    // Confirm ZSUSD in Stability Pool
    assert.equal((await stabilityPool.getTotalZSUSDDeposits()).toString(), spDeposit.toString())
  ...
  it('liquidateTroves():  emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(285, 16)), extraZSUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    const entireColl_A = (await troveManager.Troves(alice))[1].add(await troveManager.getPendingSOVReward(alice))
    const entireDebt_A = (await troveManager.Troves(alice))[0].add(await troveManager.getPendingZSUSDDebtReward(alice))
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const { zsusdAmount: zsusdAmountD } = await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: dennis } })
    const { zsusdAmount: zsusdAmountE } = await openTrove({ ICR: toBN(dec(133, 16)), extraParams: { from: erin } })
    const { zsusdAmount: zsusdAmountF } = await openTrove({ ICR: toBN(dec(111, 16)), extraParams: { from: freddy } })

    // Check list size is 4
    assert.equal((await sortedTroves.getSize()).toString(), '4')

    // Check token balances before
    assert.equal((await zsusdToken.balanceOf(dennis)).toString(), zsusdAmountD)
    assert.equal((await zsusdToken.balanceOf(erin)).toString(), zsusdAmountE)
    assert.equal((await zsusdToken.balanceOf(freddy)).toString(), zsusdAmountF)
  ...
  it("liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and SOV gain", async () => {
    // Whale provides ZSUSD to the SP
    const { zsusdAmount: W_zsusdAmount } = await openTrove({ ICR: toBN(dec(300, 16)), extraZSUSDAmount: dec(4000, 18), extraParams: { from: whale } })
    await stabilityPool.provideToSP(W_zsusdAmount, ZERO_ADDRESS, { from: whale })

    const { zsusdAmount: A_zsusdAmount, totalDebt: A_totalDebt, collateral: A_coll } = await openTrove({ ICR: toBN(dec(191, 16)), extraZSUSDAmount: dec(40, 18), extraParams: { from: alice } })
    const { zsusdAmount: B_zsusdAmount, totalDebt: B_totalDebt, collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraZSUSDAmount: dec(240, 18), extraParams: { from: bob } })
    ...
    await stabilityPool.provideToSP(A_zsusdAmount, ZERO_ADDRESS, { from: alice })
    await stabilityPool.provideToSP(B_zsusdAmount, ZERO_ADDRESS, { from: bob })

    const totalDeposit = W_zsusdAmount.add(A_zsusdAmount).add(B_zsusdAmount)
    ...
    // Check ZSUSD in Pool
    assert.equal((await stabilityPool.getTotalZSUSDDeposits()).toString(), totalDeposit)
    ...
    Total ZSUSD in Pool: 680 ZSUSD

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 100 + 300 + 100 = 500 ZSUSD
    Total liquidated SOV = 1 + 3 + 1 = 5 SOV

    Whale ZSUSD Loss: 500 * (400/680) = 294.12 ZSUSD
    Alice ZSUSD Loss:  500 *(40/680) = 29.41 ZSUSD
    Bob ZSUSD Loss: 500 * (240/680) = 176.47 ZSUSD

    Whale remaining deposit: (400 - 294.12) = 105.88 ZSUSD
    Alice remaining deposit: (40 - 29.41) = 10.59 ZSUSD
    Bob remaining deposit: (240 - 176.47) = 63.53 ZSUSD

    Whale SOV Gain: 5*0.995 * (400/680) = 2.93 SOV
    Alice SOV Gain: 5*0.995 *(40/680) = 0.293 SOV
    Bob SOV Gain: 5*0.995 * (240/680) = 1.76 SOV

    Total remaining deposits: 180 ZSUSD
    Total SOV gain: 5*0.995 SOV */

    const ZSUSDinSP = (await stabilityPool.getTotalZSUSDDeposits()).toString()
    const SOVinSP = (await stabilityPool.getSOV()).toString()

    // Check remaining ZSUSD Deposits and SOV gain, for whale and depositors whose troves were liquidated
    const whale_Deposit_After = (await stabilityPool.getCompoundedZSUSDDeposit(whale)).toString()
    ...
    const whale_SOVGain = (await stabilityPool.getDepositorSOVGain(whale)).toString()
    const alice_SOVGain = (await stabilityPool.getDepositorSOVGain(alice)).toString()
    const bob_SOVGain = (await stabilityPool.getDepositorSOVGain(bob)).toString()

    const liquidatedDebt = A_totalDebt.add(B_totalDebt).add(C_totalDebt)
    const liquidatedColl = A_coll.add(B_coll).add(C_coll)
    assert.isAtMost(th.getDifference(whale_Deposit_After, W_zsusdAmount.sub(liquidatedDebt.mul(W_zsusdAmount).div(totalDeposit))), 100000)
    assert.isAtMost(th.getDifference(alice_Deposit_After, A_zsusdAmount.sub(liquidatedDebt.mul(A_zsusdAmount).div(totalDeposit))), 100000)
    assert.isAtMost(th.getDifference(bob_Deposit_After, B_zsusdAmount.sub(liquidatedDebt.mul(B_zsusdAmount).div(totalDeposit))), 100000)

    assert.isAtMost(th.getDifference(whale_SOVGain, th.applyLiquidationFee(liquidatedColl).mul(W_zsusdAmount).div(totalDeposit)), 4000)
    assert.isAtMost(th.getDifference(alice_SOVGain, th.applyLiquidationFee(liquidatedColl).mul(A_zsusdAmount).div(totalDeposit)), 4000)
    assert.isAtMost(th.getDifference(bob_SOVGain, th.applyLiquidationFee(liquidatedColl).mul(B_zsusdAmount).div(totalDeposit)), 4000)
    ...
    assert.isAtMost(th.getDifference(total_ZSUSDinSP, totalDeposit.sub(liquidatedDebt)), 1000)
    assert.isAtMost(th.getDifference(total_SOVinSP, th.applyLiquidationFee(liquidatedColl)), 1000)
  })

  it("liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or SOV gain", async () => {
    // Whale provides 400 ZSUSD to the SP
    await openTrove({ ICR: toBN(dec(300, 16)), extraZSUSDAmount: dec(400, 18), extraParams: { from: whale } })
    await stabilityPool.provideToSP(dec(400, 18), ZERO_ADDRESS, { from: whale })

    await openTrove({ ICR: toBN(dec(182, 16)), extraZSUSDAmount: dec(170, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraZSUSDAmount: dec(300, 18), extraParams: { from: bob } })
    ...
    // Check ZSUSD and SOV in Pool  before
    ...
    assert.equal(ZSUSDinSP_Before, dec(800, 18))
    assert.equal(SOVinSP_Before, '0')
    ...
    // Check ZSUSD and SOV in Pool after
    ...
    const whale_SOVGain_After = (await stabilityPool.getDepositorSOVGain(whale)).toString()
    ...
    assert.equal(whale_SOVGain_After, '0')
    assert.equal(alice_SOVGain_After, '0')
    assert.equal(bob_SOVGain_After, '0')
  })
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains active", async () => {
    ...
    await openTrove({ ICR: toBN(dec(300, 16)), extraZSUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    That leaves 50 ZSUSD in the Pool to absorb exactly half of Carol's debt (100) */
    await troveManager.liquidateTroves(10)
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains in TroveOwners Array", async () => {
    ...
    // Whale provides ZSUSD to the SP
    const spDeposit = A_totalDebt.add(B_totalDebt).add(C_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(220, 16)), extraZSUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
  ...
  it("liquidateTroves() with a non fullfilled liquidation: still can liquidate further troves after the non-liquidated, emptied pool", async () => {
    ...
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraZSUSDAmount: D_totalDebt, extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(208, 16)), extraParams: { from: erin } })
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
     ...
     That leaves 97 ZSUSD in the Pool that won’t be enough to absorb Carol,
  ...
  it("liquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 ZSUSD in the Pool that won’t be enough to absorb any other trove */
    const tx = await troveManager.liquidateTroves(10)

    // Expect system debt reduced by 203 ZSUSD and system coll 2.3 SOV
    const entireSystemCollAfter = await troveManager.getEntireSystemColl()
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt()
  ...
  it("liquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values", async () => {
    ...
    // Whale provides ZSUSD to the SP
    const spDeposit = A_totalDebt.add(B_totalDebt).add(C_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(240, 16)), extraZSUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 ZSUSD in the Pool which won’t be enough for any other liquidation */
    const liquidationTx = await troveManager.liquidateTroves(10)

    const [liquidatedDebt, liquidatedColl, collGasComp, zsusdGasComp] = th.getEmittedLiquidationValues(liquidationTx)
    ...
    assert.equal(zsusdGasComp.toString(), dec(40, 18))
    ...
    // can claim collateral
    const alice_balanceBefore = await sovToken.balanceOf(alice)
    await borrowerOperations.claimCollateral({ from: alice, gasPrice: 0 })
    const alice_balanceAfter = await sovToken.balanceOf(alice)
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    await openTrove({ ICR: toBN(dec(426, 16)), extraZSUSDAmount: spDeposit, extraParams: { from: alice } })
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    const spDeposit = B_totalDebt.add(C_totalDebt).add(D_totalDebt).add(E_totalDebt).add(F_totalDebt)
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(426, 16)), extraZSUSDAmount: spDeposit, extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(426, 16)), extraZSUSDAmount: A_totalDebt, extraParams: { from: whale } })

    // Alice deposits ZSUSD to Stability Pool
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: alice })

    // to compensate borrowing fee
    await zsusdToken.transfer(alice, A_totalDebt, { from: whale })
    // Alice closes trove
  ...
  it("batchLiquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    // Expect system debt reduced by 203 ZSUSD and system coll by 2 SOV
    const entireSystemCollAfter = await troveManager.getEntireSystemColl()
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt()
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool ZSUSD > debt to liquidate: can liquidate troves out of order", async () => {
    ...
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(206, 16)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(280, 16)), extraZSUSDAmount: dec(500, 16), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(282, 16)), extraZSUSDAmount: dec(500, 16), extraParams: { from: freddy } })

    // Whale provides 1000 ZSUSD to the SP
    const spDeposit = A_totalDebt.add(C_totalDebt).add(D_totalDebt)
    await openTrove({ ICR: toBN(dec(219, 16)), extraZSUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool empty: doesn't liquidate any troves", async () => {
    ...
    const dennisDebt_After = (await troveManager.Troves(dennis))[0].add(await troveManager.getPendingZSUSDDebtReward(dennis))
    const bobDebt_After = (await troveManager.Troves(bob))[0].add(await troveManager.getPendingZSUSDDebtReward(bob))
    const carolDebt_After = (await troveManager.Troves(carol))[0].add(await troveManager.getPendingZSUSDDebtReward(carol))

    const dennisColl_After = (await troveManager.Troves(dennis))[1].add(await troveManager.getPendingSOVReward(dennis))  
    const bobColl_After = (await troveManager.Troves(bob))[1].add(await troveManager.getPendingSOVReward(bob))
    const carolColl_After = (await troveManager.Troves(carol))[1].add(await troveManager.getPendingSOVReward(carol))
  ...
  it('batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size', async () => {
    ...
    await openTrove({ ICR: toBN(dec(245, 16)), extraZSUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    // Whale withdraws entire deposit, and re-deposits 132 ZSUSD
    ...
    // Expected Stability Pool to fully absorb B (92 ZSUSD + 10 virtual debt), 
    // but not E as there are not enough funds in Stability Pool
    
    const stabilityBefore = await stabilityPool.getTotalZSUSDDeposits()
    ...
    const stabilityAfter = await stabilityPool.getTotalZSUSDDeposits()
  ...
  it('batchLiquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    // Whale opens trove and adds 220 ZSUSD to SP
    const spDeposit = F_totalDebt.add(G_totalDebt).add(A_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(285, 16)), extraZSUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
