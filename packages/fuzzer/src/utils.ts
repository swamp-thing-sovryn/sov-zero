...
export const createRandomTrove = (price: Decimal) => {
  ...
  if (Math.random() < 0.5) {
    ...
    const debt = ZSUSD_LIQUIDATION_RESERVE.add(truncateLastDigits(maxDebt - benford(maxDebt)));

    return new Trove(collateral, debt);
  } else {
    const debt = ZSUSD_LIQUIDATION_RESERVE.add(100 * randomValue);
...
export const randomDebtChange = ({ debt }: Trove) =>
  Math.random() < 0.5
    ? { repayZSUSD: debt.mul(1.1 * Math.random()) }
    : { borrowZSUSD: debt.mul(0.5 * Math.random()) };
...
const checks = [
  ...
  new EqualityCheck("tokensInStabilityPool", l => l.getZSUSDInStabilityPool(), decimalsEqual)
];
