...
const { Decimal, UserTrove, ZSUSD_LIQUIDATION_RESERVE } = require("@liquity/lib-base");
...
async function tryToLiquidate(liquity) {
  ...
  try {
    ...
    const expectedCompensation = total.collateral
      ...
      .add(ZSUSD_LIQUIDATION_RESERVE.mul(troves.length));
    ...
    const { collateralGasCompensation, zsusdGasCompensation, liquidatedAddresses } = receipt.details;
    ...
    const totalCompensation = collateralGasCompensation
      .mul(store.state.price)
      .add(zsusdGasCompensation);

    success(
      `Received ${bold(`${collateralGasCompensation.toString(4)} ETH`)} + ` +
        `${bold(`${zsusdGasCompensation.toString(2)} ZSUSD`)} compensation (` +
